...
Below the pipeline message, a comment will present a [staging alias](/docs/v2/custom-domains#aliasing-a-project) that updates each time the merge request is pushed to, so you can share this link with others without having to worry about sharing the latest update.
