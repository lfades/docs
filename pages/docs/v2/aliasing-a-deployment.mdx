...
export const meta = {
  ...
  editUrl: 'pages/docs/v2/aliasing-a-deployment.mdx',
...
If you want to use a specific domain with your project, you must add an alias to the project from the **Domains** section of the project dashboard.
...
To do this, select the project from the dashboard and click the **Domains** tab. Here you will find two options representing different deployment environments, **Staging** and **Production**. Choose the environment for your desired alias and enter the alias, for example, `timothy.now.sh`, into the input box.

If the domain is **registered to your account and available for use**, this will be indicated below the input box.

If the domain is **not registered to your account**, it will automatically be added and we will attempt to verify it for you.

<Note hint>You can add as many aliases to a project as you require.</Note>

Once the alias has been added, the interface will update to reflect this along with further information such as instructions on how to use it, this will be explored further later on in this document.
...
Every new deployment, whether it is from [Now CLI](/download), [Now for GitHub](/docs/v2/integrations/now-for-github), or [Now for GitLab](/docs/v2/integrations/now-for-gitlab), will receive both a unique deployment URL and a staging alias for the deployment that updates with every deployment you personally make from the project.
...
For **deployments made from a personal account** with [Now CLI](/download), you will get a staging alias like the following:
...
<Card title="Switching Scope" href="/docs/v2/platform/users-and-teams#now-cli">
  Learn how to switch between scopes using Now CLI.
</Card>
...
Deploying with the following command will trigger a deployment to the production alias defined in the projects **Domains** dashboard:

<TerminalInput>now --target production</TerminalInput>

#### With Now for Git

Each push to the default branch with either [Now for GitHub](/docs/v2/integrations/now-for-github) or [Now for GitLab](/docs/v2/integrations/now-for-gitlab), will result in aliasing a fresh deployment to the production alias, defined in the projects **Domains** dashboard.
...
The same mechanism used above can be used to configure [a **custom domain**](/docs/v2/custom-domains).
...
If `my-zeit-domain.com` is not yet yours and is available to [purchase](/docs/v2/advanced/domains/buying-a-domain), <Now color="#000"/> will allow you to buy it directly from the command-line, which is the only step you need to take. <Now color="#000"/> will take care of all the domain configuration.

Otherwise, you need to [add the domain](/docs/v2/custom-domains) to your <Now color="#000"/> account or team.
